...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def union(that: TweetSet): TweetSet =
    val newThat = if elem.text.nonEmpty then that.incl(elem) else that
  ...
  def mostRetweeted: Tweet =
    ...
    if !left.isEmpty && (leftMostRetweeted.retweets > elem.retweets) then
       if !right.isEmpty && (rightMostRetweeted.retweets > leftMostRetweeted.retweets) then
         rightMostRetweeted
       else
         leftMostRetweeted
    else if !right.isEmpty && (rightMostRetweeted.retweets > elem.retweets) then
       rightMostRetweeted
    else
       elem
